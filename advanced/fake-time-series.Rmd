...
```{r date-and-stat}
ggplot(X, aes(Date, Proportion, colour = as.factor(Cohort))) +
  geom_point(alpha = 0.5, position = position_dodge(width=20)) +
  stat_summary(fun.data = "mean_sdl", fun.args = list(mult = 1), shape = 'square', position = position_dodge(width=20)) +
  stat_summary(fun = mean, geom='line', position = position_dodge(width=20)) +
